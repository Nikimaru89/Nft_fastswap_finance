...
const headCells: HeadCell[] = [
  { id: 'creator', numeric: false, disablePadding: true, label: 'Creator' },
  { id: 'volume', numeric: true, disablePadding: false, label: 'Volume(USD)' },
...
export default function EnhancedTable({ filter = '' }) {
  ...
  const fetchRank = async () => {
    const { data } = await api.activity.getRankByCreators()
  ...
  useEffect(() => {
    if (!!drop) {
      setFiltered(drop.filter(it => it.user_info[0].name.includes(filter)))
    }
  }, [filter, drop])
  ...
  const handleNavigate = (index) => {
    navigate(`/account/${filtered[index].user_info[0].address}`)
  }
                      ...
                      >
                        <TableCell align="left">
                          <Nft>
                            ...
                            <NFTImage src={row.user_info[0].avatar || '/assets/images/avatar7.svg'} />
                            <NFTHeader>{row.user_info[0].name}</NFTHeader>
                          </Nft>
                        </TableCell>
