...
const headCells: HeadCell[] = [
  ...
  { id: 'address', numeric: true, disablePadding: false, label: 'Wallet' },
  { id: 'createdAt', numeric: true, disablePadding: false, label: 'Joined At' }
];
            ...
            <TableBody >
              {
                stableSort(acts, getComparator(order, orderBy))
                  ...
                  .map((row, index) => (
                    <TableRow key={index} style={{ cursor: 'pointer' }} onClick={() => handleNavigate(row.address)}>
                      <TableCell sx={{ display: 'flex', alignItems: 'center' }}>
                        <NFTImage src={row.avatar as string || '/assets/images/avatar7.svg'} />
                        <NFTHeader>{row.name}</NFTHeader>
                      </TableCell>
                      <TableCell>
                        <NFTHeader>{row.address}</NFTHeader>
                      </TableCell>
                      <TableCell>
                        <NFTHeader>{getTime(row.createdAt)}</NFTHeader>
                      </TableCell>
                    </TableRow>
                  ))}
              {emptyRows > 0 && (
