...
const headCells: HeadCell[] = [
  ...
  { id: 'from', numeric: true, disablePadding: false, label: 'Owner' },
];
...
const EnhancedTable = (props) => {
  ...
  const { t } = useTranslation()
  const token = useSelector((state: any) => state.face.adminAccess)
  ...
  const handleRemove = async (ind) => {
    dispatch(controlNotification({ notificationValue: true, title: t("Removing Nfts"), description: t("Please wait...") }))
    const options = {
      method: "POST",
      headers: {
    ...
    try {
      await api.admin.removeNfts(options, acts[ind].tokenId)
      await update()
    } catch (error) {
      toast("Remove nft failed")
    }
